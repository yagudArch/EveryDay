...
|---|---|---|---|---|---|---|
| OPS-001 | P0 | QA/DEVOPS | BLOCKED | FND-006 ✓ | .github/**, apps/mobile native config | BLOCKED (окружение без native toolchain). Хост Windows/MINGW64: ANDROID_HOME/ANDROID_SDK_ROOT пусты, нет adb/sdkmanager/gradle, JDK 25 (нужен 17), нет macOS/Xcode, нет eas.json. Debug APK, iOS simulator build и device cold-launch/persistence E2E НЕ выполнимы и не фабрикуются. Требуется CI-runner (ubuntu Android SDK+JDK17, macos Xcode) или окружение с SDK/устройством. Отчёт .ai/OPS-001-REPORT.md. Адресат LEAD/заказчик |
