...
| BCK-001 | P0 | BACKEND | REVIEW | FND-006 ✓, NUT-001 ✓ | database/migrations/0003_auth_hardening.sql, apps/backend/src/{auth/crypto.ts, db/repositories/{auth-tokens,users,sessions}.ts, services/{auth-hardening-service,token-delivery,auth-service}.ts, routes/auth.ts, context.ts, app.ts}, apps/backend/tests/{auth-hardening.test.ts,helpers.ts,migrations.test.ts} | РЕАЛИЗОВАНО (ожидает подтверждения LEAD). Email verification (request/confirm), password reset (request/confirm + revoke-all), revoke-all sessions; миграция 0003 (users.email_verified/at + таблица auth_tokens, только SHA-256 hash, single-use, FK CASCADE); opaque токены через инъектируемый TokenDelivery seam (не логируется/не эхонится); no user enumeration на reset request; strict credential rate-limit на новых sensitive endpoints. packages/contracts НЕ менялся. Проверка: npm run check PASS — builds contracts/AI/backend + mobile typecheck + 247 tests/26 files (+14, 0 регрессий). Git-сдача по §9/§14; DONE подтверждает LEAD. Разблокирует BCK-002 |
