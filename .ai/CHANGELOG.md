...
## 2026-09-21 — BACKEND — BCK-002 memory / privacy / account REVIEW
- Реализовано по опубликованному LEAD контракту c998b60 (packages/contracts НЕ менялся). Архитектура routes → services → repositories → database соблюдена.
- Memory CRUD: GET /api/v1/memory (200 MemoryList), POST /api/v1/memory (201 MemoryFact, source всегда 'user_confirmed'), DELETE /api/v1/memory (200 MemoryDeleteAll{deletedCount}, delete-all, идемпотентно), DELETE /api/v1/memory/{id} (204). Новый memory-service.ts; profile-data.ts расширен memory.insert/deleteOwnedById/deleteAllForUser. routes/memory.ts + registerMemoryRoutes в app.ts.
- Account export: GET /api/v1/account/export (200 AccountExport) — profile/preferences/goals/memory/subscription + generatedAt, только реальные данные, недоступные модули не выдумываются. Memory включается независимо от memoryEnabled (флаг влияет лишь на выдачу в day context, не на хранение/экспорт).
- Account deletion: DELETE /api/v1/account (204, body DeleteAccountSchema{password}) — необратимо. Требует текущий пароль (verifyPassword против stored hash), неверный → 401 и ничего не удаляется. Успех = один DELETE FROM users; FK ON DELETE CASCADE при foreign_keys=ON удаляет sessions/preferences/preference_values/goals/memory/daily_context/subscriptions/meals/nutrition_goals/auth_tokens и тем самым отзывает все сессии. account-service.ts (exportAccount, deleteAccount); users.ts (deleteById).
- БЕЗ новой миграции: таблица user_memory и все нужные FK CASCADE уже присутствуют в 0001_foundation.sql — миграция 0004 не понадобилась, fake-миграция не создавалась.
- Privacy/ownership: userId только из session principal, никогда из body/query/header; strict DTO — лишний userId → 400. Все memory-запросы scoped по user_id; чужой факт по угаданному id → 404 (не удаляется). CORS добавлен метод DELETE. DELETE /memory/{id}: контрактный {id} сохранён (memoryFactById в endpoints/OpenAPI), Fastify-роут зарегистрирован с :id; delete-all и delete-one — различные роуты.
- Тесты: apps/backend/tests/memory-account.test.ts (+13): memory CRUD, 400 на strict/empty fact, 204/404 delete-one, delete-all count+идемпотентность, 401 без auth, cross-user ownership (чтение/удаление/delete-all не пересекают пользователей), memoryEnabled vs day context, export real-data-only, deletion wrong-password 401, success 204 + физическая проверка каскада по всем таблицам + session revoke + невозможность login, isolation между пользователями, auth/body.
- Проверка: npm run check PASS — builds contracts/AI/backend + mobile typecheck + vitest 260 tests / 27 files (было 247; +13, 0 регрессий). Git-сдача по AGENTS.md §9/§14; DONE подтверждает LEAD.

## 2026-09-21 — LEAD / ARCHITECT — приёмка BCK-002
- BCK-002 (BACKEND) принята и переведена в DONE. Implementation d76f85c в origin/main (memory CRUD/delete-all, account export, необратимое удаление аккаунта). Проверено LEAD: контракт c998b60 соблюдён и не менялся (`git diff c998b60..d76f85c -- packages/contracts` пусто); memory ownership через SQL-предикат user_id (cross-user невозможен); export — только реальные данные без fake-модулей; deletion требует текущий пароль (401 без удаления при несовпадении) и каскадит по FK ON DELETE CASCADE, включая sessions → все сессии отозваны; memoryEnabled/aiConsent остаются через PATCH /preferences; npm run check PASS 260/260, working tree clean, вне BCK-002 изменений нет. Foundation-аудит не повторялся; backend-код LEAD не менял.
- Новой миграции не потребовалось (user_memory и FK CASCADE уже в 0001); fake-миграция не создавалась.
- Следующее доступное: OPS-002 (QA/DEVOPS, разблокирует AI-001). BACKEND в цикле 1 без открытых задач (NUT-004 ← NUT-003 ← AI-001). Остаются BLOCKED: OPS-001 (native), MOB-001 (←OPS-001), AI-001 (←OPS-002+credentials/privacy), NUT-002 (←MOB-001), NUT-003 (←AI-001), NUT-004 (←NUT-003).
- Синхронизированы .ai/TASKS.md, .ai/ACTIVE_WORK.md, .ai/PROJECT_STATE.md.

## 2026-09-23 — QA / DEVOPS — OPS-002 staging/secrets/backup/audit (REVIEW)
- Реализовано в зоне QA/DEVOPS (scripts/**, .github/workflows/**, deploy/**, .gitignore); apps/**, packages/**, database/**, contracts НЕ менялись. OPS-001 не трогал, AI-001 не запускал.
- Backup/restore SQLite: scripts/backup-sqlite.mjs (горячий VACUUM INTO + integrity_check + SHA-256 sidecar + ротация --keep), scripts/restore-sqlite.mjs (сверка checksum+integrity до перезаписи, safety copy *.pre-restore). Реальный E2E scripts/backup-restore.test.mjs (round-trip + tamper-rejection), добавлен в .github/workflows/ci.yml.
- Staging HTTPS: deploy/staging-nginx.conf (TLS1.2/1.3 termination → loopback backend, 80→443, HSTS, client_max_body_size 64k, X-Forwarded-*). Secret handling: deploy/staging.env.example (шаблон без секретов, TRUST_PROXY=true, AI_PROVIDER=disabled); .gitignore += deploy/*.env, deploy/**/*.env, backups/.
- Проверено: npm run check PASS 260/260 tests/27 files; backup→wipe→restore на реальной мигрированной SQLite (users 1→0→1 Alice, integrity ok, sha256 совпал); tamper restore отклонён по checksum (exit 1, live не перезаписан); git check-ignore подтвердил игнор staging.env и сохранение .example.
- Security audit: npm audit 12 moderate, 0 high/critical; 2 уникальные advisory (@vitest/mocker dev-tooling; uuid GHSA-w5hq-g745-h8pq через xcode/Expo build tooling). npm audit --omit=dev для backend и contracts/ai → 0 vulnerabilities: серверный runtime не затронут. audit fix --force (vitest 5 / expo 46) — breaking, не применялся.
- multi-instance PostgreSQL — только оценка: нужны async db-абстракция вместо синхронного node:sqlite, PG-диалект миграций, общий стор для rate limiter/auth_tokens/sessions между инстансами, pg_dump/PITR вместо VACUUM INTO, one-off data migration + PG integration suite. Отдельная задача, решение LEAD.
- Ограничения: живой HTTPS на реальном домене/staging-хосте не прогонялся (нет хоста) — проверены конфиг, скрипты и gitignore; PostgreSQL не мигрировался. Статус REVIEW; DONE подтверждает LEAD. Разблокирует AI-001 в части инфраструктуры (остаются operator credentials + privacy review вне QA/DEVOPS). Отчёт .ai/OPS-002-REPORT.md.

## 2026-09-23 — LEAD / ARCHITECT — приёмка OPS-002, разблокировка инфра-части AI-001
- OPS-002 (QA/DEVOPS) принята и переведена в DONE. Implementation 6de5b2a в origin/main. Проверено LEAD: backup/restore SQLite (VACUUM INTO + integrity_check + sha256 + ротация; restore с проверкой checksum/integrity, safety copy, отказ при tamper), E2E backup-restore.test.mjs в CI; staging HTTPS reference (nginx TLS→loopback, HSTS, 64k, X-Forwarded-*); secret handling (staging.env.example + .gitignore deploy/*.env, backups/); npm run check 260/260 PASS; audit 12 moderate 0 high/critical, backend/contracts/ai --omit=dev 0 vuln. PostgreSQL — оценка без миграции. Прикладной код не менялся; Foundation-аудит не повторялся.
- AI-001: инфраструктурная предпосылка (staging HTTPS pattern, server-side secret handling, backup/restore) снята OPS-002. Остаётся BLOCKED до двух внешних условий вне QA/DEVOPS: operator-supplied server-side credentials + privacy review. LEAD задачу не стартует.
